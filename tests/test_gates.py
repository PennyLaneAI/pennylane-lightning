...
@pytest.mark.skipif(device_name != "lightning.qubit", reason="LightningQubit required")
@pytest.mark.parametrize("theta,phi", list(zip(THETA, PHI)))
@pytest.mark.parametrize(
    "wires",
    [
...
def test_controlled_qubit_unitary(controls, wires, theta, phi, tol):
    """Test that Hadamard expectation value is correct"""
    if set(controls).intersection(wires) or set(wires).intersection(controls):
        return
    n_qubits = 8
    n_wires = len(wires)
    dev_def = qml.device("default.qubit", wires=n_qubits)
    ...
    circ = qml.QNode(circuit, dev)
    circ_def = qml.QNode(circuit, dev_def)
    # print(np.real(init_state))
    # print(np.real(circ_def()))
    # print(np.real(circ()))
    # print(np.real(circ() - circ_def()))
    # print(np.imag(circ() - circ_def()))
    assert np.allclose(circ(), circ_def(), tol)

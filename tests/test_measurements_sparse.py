...
class TestSparseExpval:
    ...
    def dev(self, request):
        return qp.device(device_name, wires=2, c_dtype=request.param)

    @pytest.mark.parametrize(
        "cases",
        [
            [qp.PauliX(0) @ qp.Identity(1), 0.00000000000000000, 1.000000000000000000],
            [qp.Identity(0) @ qp.PauliX(1), -0.19866933079506122, 0.960530638694763184],
            [qp.PauliY(0) @ qp.Identity(1), -0.38941834230865050, 0.848353326320648193],
            [qp.Identity(0) @ qp.PauliY(1), 0.00000000000000000, 1.000000119209289551],
            [qp.PauliZ(0) @ qp.Identity(1), 0.92106099400288520, 0.151646673679351807],
            [qp.Identity(0) @ qp.PauliZ(1), 0.98006657784124170, 0.039469480514526367],
        ],
    )
...
class TestSparseExpvalQChem:
    ...
    def test_sparse_pauli_words(self, qubits, wires, H, hf_state, excitations, tol, dtype):
        ...
        dev = qp.device(device_name, wires=wires, c_dtype=dtype)

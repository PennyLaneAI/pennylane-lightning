...
def test_name():
    """Tests the name of LightningQubit."""
    assert LightningQubit().name == "lightning.qubit"


@pytest.mark.skipif(LightningQubit._CPP_BINARY_AVAILABLE, reason="Only when there is no binary")
def test_name_no_binary():
    """Tests that we are offloading to the default qubit."""
    assert LightningQubit().name == "default.qubit"
...
def test_create_device_with_unsupported_dtype():
    with pytest.raises(TypeError, match="Unsupported complex Type:"):
        LightningQubit(c_dtype=np.complex256)


@pytest.mark.skipif(not LightningQubit._CPP_BINARY_AVAILABLE, reason="Lightning binary required")
def test_no_jvp_functionality():
    """Test that jvp is not supported on LightningQubit."""
    dev = LightningQubit()
...
def test_no_vjp_functionality():
    """Test that vjp is not supported on LightningQubit."""
    dev = LightningQubit()
...
@pytest.mark.skipif(not LightningQubit._CPP_BINARY_AVAILABLE, reason="Lightning binary required")
class TestTracking:
    """Testing the tracking capabilities of LightningQubit."""

    def test_tracker_set_upon_initialization(self):
        """Test that a new tracker is initialized with each device."""
        assert LightningQubit.tracker is not LightningQubit().tracker
...
class TestSupportsDerivatives:
    """Test that LightningQubit states what kind of derivatives it supports."""

    def test_supports_adjoint(self):
        """Test that LightningQubit says that it supports adjoint method."""
        dev = LightningQubit()
    ...
    def test_does_not_support_other_gradient_methods(self, gradient_method):
        """Test that LightningQubit currently does not support other gradient methods natively."""
        dev = LightningQubit()
...
class TestExecutingBatches:
    @staticmethod
    def f(phi):
        """A function that executes a batch of scripts on LightningQubit without preprocessing."""
        ops = [
            qml.PauliX("a"),
            qml.PauliX("b"),
        ...
        return LightningQubit().execute((qs1, qs2))
...
class TestSumOfTermsDifferentiability:
    ...
    def f(scale, n_wires=10, offset=0.1, convert_to_hamiltonian=False):
        ...
        return LightningQubit().execute(qs)
...
class TestPreprocessingIntegration:
    ...
    def test_preprocess_batch_circuit(self):
        ...
        post_processing_fn, config = dev.preprocess()
        batch, pre_processing_fn = post_processing_fn(initial_batch)
...
def test_broadcasted_parameter():
    """Test that LightningQubit handles broadcasted parameters as expected."""
    dev = LightningQubit()

...
from pennylane_lightning.experimental._preprocess import (
    _accepted_operator,
...
class TestExpandFnValidation:
    ...
    def test_expand_fn_invalid_tensor_observable(self):
        """Test that expand_fn throws an error when a tensor includes invalid observable"""
        tape = QuantumScript(
    ...
    def test_expand_fn_valid_tensor_observable(self):
        """Test that expand_fn passes when a tensor includes only valid observable."""
        tape = QuantumScript(
            ops=[qml.PauliX(0), qml.PauliY(1)],
    ...
    def test_state_prep_only_one(self):
        ...
        with pytest.raises(
            DeviceError, match=r"LightningQubit2 accepts at most one state prep operation."
        ):
            expand_fn(qs)

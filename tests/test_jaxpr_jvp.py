...
class TestErrors:
    ...
    def test_error_not_adjoint_method(self):
        ...
        with pytest.raises(
            ...
            qp.device(device_name, wires=1).jaxpr_jvp(jaxpr.jaxpr, args, (0.5,), execution_config)
    ...
    def test_mismatch_args_tangents(self):
        ...
        @qp.qnode(device=qp.device(device_name, wires=1))
        def circuit(x):
            qp.RX(x, 0)
            return qp.expval(qp.Z(0))
        ...
        with pytest.raises(ValueError, match="The number of arguments and tangents must match"):
            qp.device(device_name, wires=1).jaxpr_jvp(jaxpr.jaxpr, args, tangents)
    ...
    def test_only_measurements(self):
        ...
        with pytest.raises(NotImplementedError, match="The circuit should return a measurement"):
            qp.device(device_name, wires=1).jaxpr_jvp(jaxpr.jaxpr, args, (0.5,))
    ...
    def test_wrong_order(self):
        ...
        with pytest.raises(jax.errors.JaxRuntimeError) as exc_info:
            qp.device(device_name, wires=1).jaxpr_jvp(jaxpr.jaxpr, args, (0.5, 0.6))
    ...
    def test_no_classical_preprocessing(self):
        ...
        with pytest.raises(jax.errors.JaxRuntimeError) as exc_info:
            qp.device(device_name, wires=1).jaxpr_jvp(jaxpr.jaxpr, (x,), (dx,))
    ...
    def test_no_shots(self):
        ...
        with pytest.warns(
            ...
            with pytest.raises(
                ...
                qp.device(device_name, wires=1, shots=100).jaxpr_jvp(jaxpr.jaxpr, args, (0.5,))
...
class TestCorrectResults:
    ...
    def test_basic_circuit(self, use_jit):
        ...
        args = (0.82,)
        tangents = (2.0,)
        dev_jaxpr_jvp = qp.device(device_name, wires=1).jaxpr_jvp
    ...
    def test_abstract_zero_tangent(self):
        ...
        jaxpr = jax.make_jaxpr(f)(0.5)
        [results], [dresults] = qp.device(device_name, wires=1).jaxpr_jvp(
            jaxpr.jaxpr, args, tangents
        )

        assert qp.math.allclose(results, jnp.cos(0.5))
        assert qp.math.allclose(dresults, 0)
    ...
    def test_multiple_in(self):
        ...
        dev_jaxpr_jvp = qp.device(device_name, wires=2).jaxpr_jvp
    ...
    def test_multiple_in_and_out(self):
        ...
        def f(x, y):
            qp.RX(x, 0)
            qp.RY(y, 1)
            return qp.expval(qp.Y(0)), qp.expval(qp.X(1))

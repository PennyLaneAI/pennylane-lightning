...
- Fix sampling with dynamically allocated and released qubits in Catalyst. The state vector is now compacted before measurements to correctly handle released qubits, ensuring `qml.sample()` and other measurement operations return correct results when using `qml.allocate()` and `qml.release()`.

...
contribuitors_begin=$(grep -n "### Contributors" $CHANGELOG_FILE | head -n 1 | cut -d: -f1)

# Print list of Authors from Changelog and Git
echo "Authors in CHANGELOG       /  Authors in Git"
echo "---------------------------|---------------------------"
paste <(sed -n "$((contribuitors_begin + 4)),$((changelog_lower_bound - 2))p" $CHANGELOG_FILE | sort | sed 's/,//; s/$/,/') <(git log master --since=$LAST_RELEASE_DATE | grep "Author:" | sort | uniq | sed 's|<.*|| ; s|Author: ||' ) | column -t -s ','

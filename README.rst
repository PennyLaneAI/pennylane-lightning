...
* ``lightning.tensor``: is a tensor network simulator based on the `NVIDIA cuQuantum SDK <https://developer.nvidia.com/cuquantum-sdk>`_ (requires NVDIA GPUs with SM 7.0 or greater). This device is designed to simulate large-scale quantum circuits using tensor networks. For small circuits, state-vector simulator plugins may be more suitable. The supported method is Matrix Product State (MPS). This device does not currently support finite shots. At present, the supported measurement types are expectation values and the supported differentiation methods are ``parameter-shift`` or ``finite-diff``. Note that only 1,2-wires gates and gates that can be decomposed by PennyLane into 1,2-wires gates are supported.

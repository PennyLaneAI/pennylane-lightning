...
class LightningQubit(QubitDevice):
    ...
    def _apply_state_vector(self, state, device_wires):
        ...
        print("(dim,)", (dim,), self.num_wires, 1 << self.num_wires)
        ...
        if len(device_wires) == self.num_wires and Wires(sorted(device_wires)) == device_wires:
            ...
            print("self._state", np.shape(self._state))
            print("return?")
            return
    ...
    def apply_lightning(self, state, operations):
        ...
        """
        print("===> Apply lightning")
        batch_size_op = self.operations_batch_size(operations)
        batch_size_op = (
            batch_size_op if batch_size_op else 1
        )  # If batch_size_op == None, we make it 1.

        print("pointer state", state.__array_interface__["data"])
        batched_states = np.repeat(state[np.newaxis, ...], batch_size_op, axis=0)
        print("pointer batched_states", batched_states.__array_interface__["data"])
        for batch_ind in range(batch_size_op):
            state = batched_states[batch_ind]
            ...
            for o in operations:
                ...
                if method is None:
                    ...
                    inv = o.inverse
                    param = o.parameters[0][batch_ind] if o.batch_size else o.parameters
                    param = (
                        [
                            param,
                        ]
    ...
    def apply(self, operations, rotations=None, **kwargs):
        ...
        if operations:  # make sure operations[0] exists
            print("operations[0].name", operations[0].name)
    ...
    def generate_samples(self):
        ...
        print("===> generate_samples lightning")
        ...
        # For the final result we'll squeeze out the batch dimension if 1.
        dim = 1 << self.num_wires
        batch_size = self._get_batch_size(self._state, (2,) * self.num_wires, dim)
        if batch_size == None:
            batch_size = 1
        print("batch_size", batch_size)
        print(np.shape(batched_samples))
        batched_samples = (
            np.squeeze(batched_samples, axis=0) if (batch_size == 1) else batched_samples
        )
        print(np.shape(batched_samples))
        print("===> generate_samples lightning")
    ...
    def expval(self, observable, shot_range=None, bin_size=None):
        ...
        batched_expval = []
        print("===> expval lightning")
        for state in self._pre_rotated_state:
            print("===> expval loop start")
            print("state", state)
            state = np.ravel(state)

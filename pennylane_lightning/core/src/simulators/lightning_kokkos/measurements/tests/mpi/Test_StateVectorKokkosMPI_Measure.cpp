...
TEMPLATE_TEST_CASE("Expval - 1-wire matrix", "[LKMPI_Expval]", float, double) {
    ...
    std::vector<Kokkos::complex<TestType>> mat_ob(exp2(num_wires * 2), {0.0,
0.0}); for (std::size_t i = 0; i < mat_ob.size(); i++) { mat_ob[i] = i *
Kokkos::complex<TestType>(0.2, 1.1);
    }
...
float, double) { const TestType EP = 1e-4; const std::size_t num_qubits = 4;
    StateVectorKokkosMPI<TestType> sv{num_qubits};
    StateVectorKokkos<TestType> sv_ref{num_qubits};
    // Only run with 4 mpi ranks:
    REQUIRE(sv.getMPISize() == 4);
...
float, double) { const TestType EP = 1e-4; const std::size_t num_qubits = 6;
    StateVectorKokkosMPI<TestType> sv{num_qubits};
    StateVectorKokkos<TestType> sv_ref{num_qubits};
    // Only run with 4 mpi ranks:
    REQUIRE(sv.getMPISize() == 4);
...
Kokkos::complex<TestType>(0.2, 1.1);
    ...
    auto ob = HermitianObs<StateVectorKokkosMPI<TestType>>(mat_ob,  {wire_0,
wire_1}); auto ob_ref = HermitianObs<StateVectorKokkos<TestType>>(mat_ob,
{wire_0, wire_1}); auto res = m.expval(ob); auto res_ref = m_ref.expval(ob_ref);
    CHECK(res == Approx(res_ref).margin(EP));
}
}
...
double) { const TestType EP = 1e-4; const std::size_t num_qubits = 4;
    StateVectorKokkosMPI<TestType> sv{num_qubits};
    StateVectorKokkos<TestType> sv_ref{num_qubits};
    // Only run with 4 mpi ranks:
    REQUIRE(sv.getMPISize() == 4);
...
double) { const TestType EP = 1e-4; const std::size_t num_qubits = 6;
    StateVectorKokkosMPI<TestType> sv{num_qubits};
    StateVectorKokkos<TestType> sv_ref{num_qubits};
    // Only run with 4 mpi ranks:
    REQUIRE(sv.getMPISize() == 4);
...
Kokkos::complex<TestType>(0.2, 1.1);
    ...
    auto ob = HermitianObs<StateVectorKokkosMPI<TestType>>(mat_ob,  {wire_0,
wire_1}); auto ob_ref = HermitianObs<StateVectorKokkos<TestType>>(mat_ob,
{wire_0, wire_1}); auto res = m.var(ob); auto res_ref = m_ref.var(ob_ref);
    CHECK(res == Approx(res_ref).margin(EP));
}
}

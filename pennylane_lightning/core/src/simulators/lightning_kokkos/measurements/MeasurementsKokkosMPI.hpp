        ...
        const Kokkos::complex<PrecisionT> *coeffs) -> PrecisionT {
            ...
            for (std::size_t word = 0; word < pauli_words.size(); word++) {
                ...
                for (std::size_t i = 0; i < target_wires[word].size(); i++) {
                    ...
                    // DO SWAP HERE
                    this->_statevector.swap_global_local_wires(global_wires_need_to_swap, local_wires_to_swap);
                    ...
                    std::size_t global_index = this->_statevector.get_global_index_from_mpi_rank(this->_statevector.get_mpi_rank());
                    for (std::size_t i = 0; i < global_Z_wires.size(); i++) {
                        std::size_t distance = std::distance(this->_statevector.global_wires_.begin(), std::find(this->_statevector.global_wires_.begin(), this->_statevector.global_wires_.end(), global_Z_wires[i]));
                        global_z_mask |= (1U << (this->_statevector.get_num_global_wires() - 1 - distance));
                    }
                    std::cout << "I am rank " << this->_statevector.get_mpi_rank() << " and BEFORE multiplying -1s my local expval = " << local_expval << std::endl;
                    if (std::popcount(global_index & global_z_mask) % 2 == 1) {
                        local_expval *= -1.0;
                    }
                    std::cout << "I am rank " << this->_statevector.get_mpi_rank() << " and AFTER multiplying -1s my local expval = " << local_expval << std::endl;

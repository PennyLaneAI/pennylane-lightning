...
TEMPLATE_TEST_CASE("StateVectorLQubitDynamic::StateVectorLQubitDynamic",
    ...
    SECTION("Aligned 256bit statevector") {
        ...
        StateVectorLQubitDynamic<PrecisionT> sv(4, Threading::SingleThread,
                                                memory_model);
        /* Even when we allocate 256 bit aligend memory, it is possible that the
         * alignment happens to be 512 bit */
        CHECK(((getMemoryModel(sv.getDataVector().data()) ==
...
TEMPLATE_TEST_CASE("StateVectorLQubitDynamic::applyMatrix with a pointer",
    ...
    SECTION("Test with different number of wires") {
        ...
        for (size_t num_wires = 1; num_wires < num_qubits; num_wires++) {
            ...
            const auto m =
                Pennylane::Util::randomUnitary<PrecisionT>(re, num_wires);
            sv1.applyMatrix(m, wires);
            Gates::GateImplementationsLM::applyMultiQubitOp<PrecisionT>(
                sv2.getData(), num_qubits, m.data(), wires, false);
            CHECK(sv1.getDataVector() ==
                  approx(sv2.getDataVector()).margin(PrecisionT{1e-5}));
        }
    }
}

TEMPLATE_TEST_CASE("StateVectorLQubitDynamic::applyOperations",
                   "[StateVectorLQubitDynamic]", float, double) {
    using PrecisionT = TestType;
    ...
    SECTION("applyOperations without params works as expected") {
        ...
        sv1.updateData(Pennylane::Util::createRandomStateVectorData<PrecisionT>(
            re, num_qubits));

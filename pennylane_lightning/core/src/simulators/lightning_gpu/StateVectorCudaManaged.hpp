...
void applyMatrix(const std::vector<std::complex<PrecisionT>> &gate_matrix,
                 const std::vector<std::size_t> &wires, bool adjoint = false) {
...
inline void applyPhaseShift(const std::vector<std::size_t> &wires, bool adjoint,
                            Precision param) {
    static const std::string name{"PhaseShift"};
    const auto gate_key = std::make_pair(name, param);
    if (!gate_cache_.gateExists(gate_key)) {
        gate_cache_.add_gate(gate_key, cuGates::getPhaseShift<CFP_t>(param));
    }
...
inline void applyIsingXX(const std::vector<std::size_t> &wires, bool adjoint,
                         Precision param) {
    static const std::vector<std::string> names(wires.size(), {"RX"});
    applyParametricPauliGate_(names, {}, wires, param, adjoint);
}
inline void applyIsingYY(const std::vector<std::size_t> &wires, bool adjoint,
                         Precision param) {
    static const std::vector<std::string> names(wires.size(), {"RY"});
    applyParametricPauliGate_(names, {}, wires, param, adjoint);
}
inline void applyIsingZZ(const std::vector<std::size_t> &wires, bool adjoint,
                         Precision param) {
    static const std::vector<std::string> names(wires.size(), {"RZ"});
    applyParametricPauliGate_(names, {}, wires, param, adjoint);
}
inline void applyIsingXY(const std::vector<std::size_t> &wires, bool adjoint,
                         Precision param) {
    ...
    applyDeviceMatrixGate_(gate_cache_.get_gate_device_ptr(gate_key), {}, wires,
                           adjoint);
}
...
inline void applySingleExcitationMinus(const std::vector<std::size_t> &wires,
                                       bool adjoint, Precision param) {
    static const std::string name{"SingleExcitationMinus"};
    const auto gate_key = std::make_pair(name, param);
    if (!gate_cache_.gateExists(gate_key)) {
        gate_cache_.add_gate(gate_key,
                             cuGates::getSingleExcitationMinus<CFP_t>(param));
    }
    applyDeviceMatrixGate_(gate_cache_.get_gate_device_ptr(gate_key), {}, wires,
                           adjoint);
}
inline void applySingleExcitationPlus(const std::vector<std::size_t> &wires,
    ...
    if (!gate_cache_.gateExists(gate_key)) {
        gate_cache_.add_gate(gate_key,
                             cuGates::getSingleExcitationPlus<CFP_t>(param));
    }
    applyDeviceMatrixGate_(gate_cache_.get_gate_device_ptr(gate_key), {}, wires,
                           adjoint);
}
...
inline void applyDoubleExcitationMinus(const std::vector<std::size_t> &wires,
                                       bool adjoint, Precision param) {
...
/* Multi-qubit gates */
inline void applyMultiRZ(const std::vector<std::size_t> &wires, bool adjoint,
                         Precision param) {
...
inline PrecisionT applyGeneratorIsingXX(const std::vector<std::size_t> &wires,
                                        bool adjoint) {
    static const std::string name{"GeneratorIsingXX"};
    static const Precision param = 0.0;
    const auto gate_key = std::make_pair(name, param);
    if (!gate_cache_.gateExists(gate_key)) {
        gate_cache_.add_gate(gate_key, cuGates::getGeneratorIsingXX<CFP_t>());
    }
    applyDeviceMatrixGate_(gate_cache_.get_gate_device_ptr(gate_key), {}, wires,
                           adjoint);
    return -static_cast<PrecisionT>(0.5);
}
inline PrecisionT applyGeneratorIsingYY(const std::vector<std::size_t> &wires,
                                        bool adjoint) {
    static const std::string name{"GeneratorIsingYY"};
    static const Precision param = 0.0;
    const auto gate_key = std::make_pair(name, param);
    if (!gate_cache_.gateExists(gate_key)) {
        gate_cache_.add_gate(gate_key, cuGates::getGeneratorIsingYY<CFP_t>());
    }
    applyDeviceMatrixGate_(gate_cache_.get_gate_device_ptr(gate_key), {}, wires,
                           adjoint);
    return -static_cast<PrecisionT>(0.5);
}
inline PrecisionT applyGeneratorIsingZZ(const std::vector<std::size_t> &wires,
                                        bool adjoint) {
    static const std::string name{"GeneratorIsingZZ"};
    static const Precision param = 0.0;
    const auto gate_key = std::make_pair(name, param);
    if (!gate_cache_.gateExists(gate_key)) {
        gate_cache_.add_gate(gate_key, cuGates::getGeneratorIsingZZ<CFP_t>());
    }
    applyDeviceMatrixGate_(gate_cache_.get_gate_device_ptr(gate_key), {}, wires,
                           adjoint);
    return -static_cast<PrecisionT>(0.5);
...
inline PrecisionT applyGeneratorIsingXY(const std::vector<std::size_t> &wires,
                                        bool adjoint) {
    static const std::string name{"GeneratorIsingXY"};
    static const Precision param = 0.0;
    const auto gate_key = std::make_pair(name, param);
    if (!gate_cache_.gateExists(gate_key)) {
        gate_cache_.add_gate(gate_key, cuGates::getGeneratorIsingXY<CFP_t>());
    }
    applyDeviceMatrixGate_(gate_cache_.get_gate_device_ptr(gate_key), {}, wires,
                           adjoint);
    return static_cast<PrecisionT>(0.5);
...
applyGeneratorSingleExcitation(const std::vector<std::size_t> &wires,
    ...
    if (!gate_cache_.gateExists(gate_key)) {
        gate_cache_.add_gate(gate_key,
                             cuGates::getGeneratorSingleExcitation<CFP_t>());
    }
    applyDeviceMatrixGate_(gate_cache_.get_gate_device_ptr(gate_key), {}, wires,
                           adjoint);
    return -static_cast<PrecisionT>(0.5);
...
applyGeneratorDoubleExcitation(const std::vector<std::size_t> &wires,
    ...
    if (!gate_cache_.gateExists(gate_key)) {
        gate_cache_.add_gate(gate_key,
                             cuGates::getGeneratorDoubleExcitation<CFP_t>());
    }
    applyDeviceMatrixGate_(gate_cache_.get_gate_device_ptr(gate_key), {}, wires,
                           adjoint);
    return -static_cast<PrecisionT>(0.5);
...
inline PrecisionT applyGeneratorMultiRZ(const std::vector<std::size_t> &wires,
                                        bool adjoint) {
    static const std::string name{"PauliZ"};
    static const Precision param = 0.0;
 ...
 */
auto getCublasCaller() const -> const CublasCaller & { return *cublascaller_; }
...
const GMap controlled_generator_map_{
    ...
    {"GlobalPhase", [&](auto &&controlled_wires, auto &&controlled_values,
                        auto &&wires, auto &&adjoint) {
...
void setBasisState_(const std::complex<Precision> &value,
    ...
    setBasisState_CUDA(BaseType::getData(), value_cu, index, async, stream_id);
}
...
void setStateVector_(const index_type num_indices,
    ...
    setStateVector_CUDA(BaseType::getData(), num_elements, d_values.getData(),
...
void applyParametricPauliGate_(const std::vector<std::string> &pauli_words,
                               ...
                               std::vector<std::size_t> tgts, Precision param,
                               bool use_adjoint = false) {
    // Transform indices between PL & cuQuantum ordering
    auto ctrlsInt =
        NormalizeCastIndices<std::size_t, int>(ctrls, BaseType::getNumQubits());
    auto tgtsInt =
        NormalizeCastIndices<std::size_t, int>(tgts, BaseType::getNumQubits());
...
void applyParametricPauliGeneralGate_(
    ...
    const std::vector<int> &ctrlsInt, const std::vector<int> &ctrls_valuesInt,
    const std::vector<int> tgtsInt, Precision param, bool use_adjoint = false) {

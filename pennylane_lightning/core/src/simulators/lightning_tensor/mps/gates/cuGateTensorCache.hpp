 ...
 */
template <class PrecisionT> class GateTensorCache {
  public:
    ...
    GateTensorCache(GateTensorCache &&other) = delete;
    GateTensorCache(bool populate, int device_id = 0,
    ...
    GateTensorCache(bool populate, const DevTag<int> &device_tag)
        : device_tag_{device_tag}, total_alloc_bytes_{0} {
        if (populate) {
            defaultPopulateCache();
        }
    }
    virtual ~GateTensorCache(){};
    ...
    void defaultPopulateCache() {
        ...
        host_gates_.emplace(
            ...
            std::forward_as_tuple(std::vector<CFP_t>{
                cuUtil::ONE<CFP_t>(), cuUtil::ZERO<CFP_t>(),
                cuUtil::ZERO<CFP_t>(),
                cuUtil::ConstMultSC(cuUtil::SQRT2<PrecisionT>() / 2.0,
        ...
        for (const auto &[h_gate_k, h_gate_v] : host_gates_) {
            ...
            device_gates_.at(h_gate_k).getDataBuffer().CopyHostDataToGpu(
                h_gate_v.data(), h_gate_v.size());
    ...
    void add_gate(const std::string &gate_name, PrecisionT gate_param,
        ...
        // TODO Construct cuDeviceTensor with rank, modes, extents, dev_tag,
        // device_alloc

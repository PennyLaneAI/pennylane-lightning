...
TEMPLATE_TEST_CASE("MPSTNCuda::applyMPO::2+_wires", "[MPSTNCuda_Param]",
                   float, double) {
    const bool inverse = GENERATE(false, true);
    {
        using cp_t = std::complex<TestType>;
        ...
        std::vector<std::vector<cp_t>> mpo_double_excitation;

        // in-order decomposition of the double excitation operator
        std::vector<cp_t> mpo_double_excitation_0 = {
            cp_t{-2.82798538, 0.0}, 
            cp_t{0., 0.0}, 
            {0.3, 0.0}, {0.4, 0.0}};
        ...
        SECTION("DoubleExcitation gate") {
            ...
            std::vector<std::vector<TestType>> mpo_double_excitation{

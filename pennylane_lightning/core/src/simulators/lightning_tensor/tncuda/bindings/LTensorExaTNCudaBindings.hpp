...
using Pennylane::LightningTensor::TNCuda::ExaTNCuda;
} // namespace
/// @endcond
...
using TensorNetBackendsExaTN =
    Pennylane::Util::TypeList<ExaTNCuda<float>, ExaTNCuda<double>, void>;
 ...
 */
template <class TensorNet, class PyClass>
void registerBackendClassSpecificBindingsExaTN(PyClass &pyclass) {
 ...
 */
void registerBackendSpecificInfoExaTN(py::module_ &m) {
    m.def("backend_info", &getBackendInfo, "Backend-specific information.");
    registerCudaUtils(m);
}

} // namespace Pennylane::LightningTensor::ExaTNCuda

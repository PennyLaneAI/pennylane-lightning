...
template <typename PrecisionT> class applyMultiRZFunctor {
  ...
  public:
    ...
    KOKKOS_FUNCTION void operator()(const std::size_t k) const {
        arr(k) *= (Kokkos::Experimental::popcount_builtin(k & wires_parity) % 2 == 0)
                      ? shift_0
                      : shift_1;
    }
};
...
void applyNCMultiRZ(Kokkos::View<Kokkos::complex<PrecisionT> *> arr_,
    ...
    auto core_function = KOKKOS_LAMBDA(
        ...
        arr(index + offset) *=
            (Kokkos::Experimental::popcount_builtin((index + offset) & wires_parity) % 2 == 0)
                ? shift_0
                : shift_1;
    };
...
void applyPauliRot(Kokkos::View<Kokkos::complex<PrecisionT> *> arr_,
    ...
    Kokkos::parallel_for(
        ...
        KOKKOS_LAMBDA(std::size_t i0) {
            ...
            if (i0 <= i1) {
                const auto count_y = Kokkos::Experimental::popcount_builtin(i0 & mask_y) * 2;
                const auto count_z = Kokkos::Experimental::popcount_builtin(i0 & mask_z) * 2;

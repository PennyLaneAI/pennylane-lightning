...
public:
  ...
  [[nodiscard]] auto createNamedObs(ObsId obsId,
    ...
    this->observables_.push_back(std::make_pair(
        std::make_shared<
            Pennylane::LightningKokkos::Observables::NamedObs<StateVectorT>>(
            obs_str, wires),
        ObsType::Basic));
  ...
  createHermitianObs(const std::vector<std::complex<PrecisionT>> &matrix,
    ...
    this->observables_.push_back(std::make_pair(
        std::make_shared<Pennylane::LightningKokkos::Observables::HermitianObs<
            StateVectorT>>(
            Pennylane::LightningKokkos::Observables::HermitianObs<StateVectorT>{
                matrix_k, wires}),
        ObsType::Basic));
   ...
   * @return ObsIdType
   */
  [[nodiscard]] auto createTensorProdObs(const std::vector<ObsIdType> &obsKeys)
      -> ObsIdType {
    ...
    this->observables_.push_back(
        std::make_pair(Pennylane::LightningKokkos::Observables::TensorProdObs<
  ...
  [[nodiscard]] auto createHamiltonianObs(const std::vector<PrecisionT> &coeffs,
                                          const std::vector<ObsIdType> &obsKeys)
      -> ObsIdType {
    const auto key_size = obsKeys.size();
    const auto obs_size = this->observables_.size();
    ...
    this->observables_.push_back(std::make_pair(
        std::make_shared<
            Pennylane::LightningKokkos::Observables::Hamiltonian<StateVectorT>>(
            Pennylane::LightningKokkos::Observables::Hamiltonian<StateVectorT>(

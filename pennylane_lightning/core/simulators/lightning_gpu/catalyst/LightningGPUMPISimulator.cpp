...
#include "LightningGPUMPISimulator.hpp"

namespace Catalyst::Runtime::Simulator {

auto LightningGPUMPISimulator::AllocateQubit() -> QubitIdType {
...
auto LightningGPUMPISimulator::AllocateQubits(std::size_t num_qubits)
...
void LightningGPUMPISimulator::ReleaseQubit(QubitIdType q) {
...
void LightningGPUMPISimulator::ReleaseQubits(const std::vector<QubitIdType> &ids) {
...
void LightningGPUMPISimulator::SetDeviceShots(std::size_t shots) {
    this->device_shots = shots;
}

auto LightningGPUMPISimulator::GetDeviceShots() const -> std::size_t {
    return this->device_shots;
}

void LightningGPUMPISimulator::SetDevicePRNG(std::mt19937 *gen) {
    this->gen = gen;
}

void LightningGPUMPISimulator::SetState(DataView<std::complex<double>, 1> &data,
...
void LightningGPUMPISimulator::MatrixOperation(
...
auto LightningGPUMPISimulator::Observable(
...
auto LightningGPUMPISimulator::TensorObservable(const std::vector<ObsIdType> &obs)
    -> ObsIdType {
    return this->obs_manager.createTensorProdObs(obs);
}

auto LightningGPUMPISimulator::HamiltonianObservable(
    const std::vector<double> &coeffs, const std::vector<ObsIdType> &obs)
    -> ObsIdType {
    return this->obs_manager.createHamiltonianObs(coeffs, obs);
}

auto LightningGPUMPISimulator::Expval(ObsIdType obsKey) -> double {
...
auto LightningGPUMPISimulator::Var(ObsIdType obsKey) -> double {
    RT_FAIL_IF(!this->obs_manager.isValidObservables({obsKey}),
               "Invalid key for cached observables");
...
void LightningGPUMPISimulator::State(DataView<std::complex<double>, 1> &state) {
...
void LightningGPUMPISimulator::Probs(DataView<double, 1> &probs) {
    Pennylane::LightningGPU::Measures::Measurements<StateVectorT> m{
        *(this->device_sv)};
...
void LightningGPUMPISimulator::PartialProbs(
...
std::vector<size_t> LightningGPUMPISimulator::GenerateSamples(size_t shots) {
...
void LightningGPUMPISimulator::Sample(DataView<double, 2> &samples) {
...
void LightningGPUMPISimulator::PartialSample(
...
void LightningGPUMPISimulator::Counts(DataView<double, 1> &eigvals,
...
void LightningGPUMPISimulator::PartialCounts(
...
auto LightningGPUMPISimulator::Measure(QubitIdType wire,
...
auto LightningGPUMPISimulator::PauliMeasure(const std::string &pauli_word,
                                         const std::vector<QubitIdType> &wires)
    -> Result {
...
void LightningGPUMPISimulator::Gradient(
...
/// LCOV_EXCL_START
GENERATE_DEVICE_FACTORY(LightningGPUMPISimulator,
                        Catalyst::Runtime::Simulator::LightningGPUMPISimulator);
/// LCOV_EXCL_STOP

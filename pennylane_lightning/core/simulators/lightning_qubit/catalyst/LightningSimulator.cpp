  ...
  for (uint8_t i = 0; i < 2; i++) {
    for (uint8_t j = 0; j < 2; j++) {
      ...
      for (size_t k = 0; k < (sv_size / 2); k++) {
        size_t idx_wire_0 = ((upper_mask & k) << 1UL) + (lower_mask & k);
...
void LightningSimulator::checkReleasedQubitsDisentangled() {
  ...
  // Check if purity is close to 1 (disentangled qubits should have purity =
  // 1)
  constexpr double epsilon = 1e-6;
  if (std::abs(purity - 1.0) > epsilon) {
    const std::string error_msg = "Cannot release qubits: released qubits are "

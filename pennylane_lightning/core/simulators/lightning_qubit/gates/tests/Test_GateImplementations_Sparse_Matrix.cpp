...
TEST_CASE("GateImplementationsLM::applyNCMultiQubitSparseOp throws if inverse "
    ...
    REQUIRE_THROWS_WITH(GateImplementationsLM::applyNCMultiQubitSparseOp(
                            state.data(), num_qubits, row_map.data(),
                            col_idx.data(), values.data(), controlled_wires,
                            controlled_values, wires, inverse),
                        Catch::Matchers::ContainsSubstring(
                            "Inverse not implemented for sparse ops."));
}

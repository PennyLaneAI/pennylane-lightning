 ...
 * @param apply_operations Assume the given state as an input if true
...
static auto StatevectorVJP(const JacobianData<PrecisionT>& jd,
    ...
    std::vector<PrecisionT> vjp(trainable_params.size());

    // Create $U_{1:p}\vert \lambda \rangle$
    StateVectorManaged<PrecisionT> lambda(jd.getPtrStateVec(), jd.getSizeStateVec());

    // Apply given operations to statevector if requested
    if (apply_operations) {
        applyOperations(lambda, ops);
    }
    StateVectorManaged<PrecisionT> mu(vec);
    StateVectorManaged<PrecisionT> mu_d(Util::log2PerfectPower(jd.getSizeStateVec()));

    const auto tp_rend = trainable_params.rend();
    auto tp_it = trainable_params.rbegin();
    size_t current_param_idx = num_param_ops - 1; // total number of parametric ops
        ...
        if (ops.hasParams(op_idx)) {
            if (current_param_idx == *tp_it) {
                ...
                vjp[trainable_param_idx] = 
                    ...
                    innerProdC(mu_d.getDataVector(), lambda.getDataVector());
                --trainable_param_idx;
                ++tp_it;
            }
            --current_param_idx;
        }
        applyOperationsAdj(lambda, ops, static_cast<size_t>(op_idx));
        applyOperationsAdj(mu, ops, static_cast<size_t>(op_idx));
    }
    return vjp;
};
} // namespace Pennylane::Algorithms

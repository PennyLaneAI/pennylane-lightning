...
if __name__ == '__main__':
    ...
    if len(res_files) == 0:
        print(f"Cannot find results for {gate_name}. Check the results are obtained from run_gate_benchmark.sh.")
        sys.exit(1)
    ...
    for kernel_idx, (kernel_name, res_file) in enumerate(res_files):
        ...
        plt.bar(n_qubits + 0.8*(kernel_idx - num_kernels/2 + 1/2), times, 
                label = kernel_name)

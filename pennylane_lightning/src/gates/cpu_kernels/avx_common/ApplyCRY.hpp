...
template <typename PrecisionT, size_t packed_size> struct ApplyCRY {
    ...
    static void applyInternalInternal(std::complex<PrecisionT> *arr,
                                      ...
                                      bool inverse,
                                      ParamT angle) {
        constexpr static auto perm = applyInternalInternalPermutation<control, target>();

        if (inverse) {
            angle *= -1.0;
        }

        const auto off_diag_factor = applyInternalInternalOffDiagFactor<control, target>(angle);
        const auto diag_factor = applyInternalInternalDiagFactor<control, target>(angle);

        for (size_t n = 0; n < exp2(num_qubits); n += packed_size / 2) {
            const auto v = PrecisionAVXConcept::load(arr + n);
            const auto diag_w = diag_factor * v;
            const auto off_diag_w = off_diag_factor * Permutation::permute<perm>(v);
    ...
    static void applyInternalExternal(std::complex<PrecisionT> *arr,
        ...
        const auto off_diag_factor = applyInternalExternalOffDiag<control>(angle);
        ...
        for (size_t k = 0; k < exp2(num_qubits - 1); k += packed_size / 2) {
            ...
            PrecisionAVXConcept::store(arr + i0, diag_factor*v0 - off_diag_factor * v1);
            PrecisionAVXConcept::store(arr + i1, diag_factor*v1 + off_diag_factor * v0);
        }
    }
    ...
    static void applyExternalInternal(std::complex<PrecisionT> *arr,
        ...
        const auto offdiag_factor = applyExternalInternalOffDiagFactor<target>(angle);
        ...
        for (size_t k = 0; k < exp2(num_qubits - 1); k += packed_size / 2) {
            ...
            const auto v1 = PrecisionAVXConcept::load(arr + i1); // control bit is 1
            const auto w1 = Permutation::permute<perm>(v1);
            PrecisionAVXConcept::store(arr + i1, diag_factor*v1 + offdiag_factor * w1);
        }
    }
    ...
    static void applyExternalExternal(std::complex<PrecisionT> *arr,
        ...
        const auto sin_factor = set1<PrecisionT, packed_size>(std::sin(angle/2));
        ...
        for (size_t k = 0; k < exp2(num_qubits - 2); k += packed_size / 2) {
            ...
            PrecisionAVXConcept::store(arr + i10, cos_factor*v10-sin_factor*v11);
            PrecisionAVXConcept::store(arr + i11, sin_factor*v10+cos_factor*v11);
        }
    }
};
} // namespace Pennylane::Gates::AVXCommon

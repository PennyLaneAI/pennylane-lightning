 ...
 *
 * This file tests gate generators. To be specific, we test whether each generator 
 * satisfies
 * @rst
 ...
 */
constexpr std::string_view remove_prefix(const std::string_view& str, size_t len) {
...
constexpr auto findGateOpForGenerator()
    -> GateOperation {
    constexpr auto gntr_name = remove_prefix(static_lookup<gntr_op>(Constant::generator_names), 9);
...
/**
 * @brief Array of all generator operations with the corresponding gate operations.
 */
constexpr static auto generator_gate_pairs = Util::tuple_to_array(generatorGatePairsIter<0>());
...
void testGeneratorForGate(RandomEngine& re, size_t num_qubits) {
    ...
    DYNAMIC_SECTION("Test generator of " << gate_name << " for kernel " <<
        ...
        auto gntr_func = GeneratorOpToMemberFuncPtr<PrecisionT, GateImplementation,
                gntr_op>::value;
        auto gate_func = GateOpToMemberFuncPtr<PrecisionT, ParamT, GateImplementation,
                gate_op>::value;
        ...
        std::transform(diff_st_1.cbegin(), diff_st_1.cend(), diff_st_2.cbegin(),
                gate_der_st.begin(),
                [](ComplexPrecisionT a, ComplexPrecisionT b){
                    return a - b;
                });
...
void testAllGeneratorForKernel(RandomEngine& re, size_t num_qubits) {
    if constexpr (gntr_idx < GateImplementation::implemented_generators.size()) {
        ...
        testGeneratorForGate<PrecisionT, ParamT, GateImplementation,
            gntr_op>(re, num_qubits);
        testAllGeneratorForKernel<PrecisionT, ParamT, GateImplementation, gntr_idx+1>(re, num_qubits);
    }
}

template <typename PrecisionT, typename ParamT, class TypeList, class RandomEngine>
void testAllGeneratorsAndKernels(RandomEngine& re, size_t num_qubits) {
    if constexpr (!std::is_same_v<TypeList, void>) {
        ...
        testAllGeneratorForKernel<PrecisionT, ParamT, GateImplementation, 0>(re, num_qubits);
        testAllGeneratorsAndKernels<PrecisionT, ParamT, typename TypeList::Next>(re, num_qubits);
    }
}

TEMPLATE_TEST_CASE("Test all generators of all kernels", "[GateImplementations_Generator]",
        float, double) {

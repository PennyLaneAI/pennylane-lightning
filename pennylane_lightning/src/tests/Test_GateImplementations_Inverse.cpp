...
template <typename PrecisionT, typename ParamT, 
          class GateImplementation, GateOperation gate_op, class RandomEngine>
void testInverseKernelGate(RandomEngine& re, size_t num_qubits) {
    ...
    if constexpr (gate_op != GateOperation::Matrix) {
        ...
        INFO("Test inverse of " << gate_name << " for kernel " << GateImplementation::name);
        const auto ini_st = create_random_state<PrecisionT>(re, num_qubits);

        auto st = ini_st;

        const auto func_ptr = GateOpToMemberFuncPtr<PrecisionT, ParamT, GateImplementation, gate_op>::value;
...
void testKernelInversesIter(RandomEngine& re, size_t num_qubits) {
    if constexpr (gate_idx < GateImplementation::implemented_gates.size()) {
        testInverseKernelGate<PrecisionT, ParamT,
                GateImplementation, GateImplementation::implemented_gates[gate_idx]>
                    (re, num_qubits);
        testKernelInversesIter<PrecisionT, ParamT, GateImplementation, gate_idx+1>(re, num_qubits);
    }
}

template <typename PrecisionT, typename ParamT, class GateImplementation, class RandomEngine>
void testKernelInverses(RandomEngine& re, size_t num_qubits) {
    testKernelInversesIter<PrecisionT, ParamT, GateImplementation, 0>(re, num_qubits);
}

template <typename PrecisionT, typename ParamT, typename TypeList, class RandomEngine>
void testKernels(RandomEngine& re, size_t num_qubits) {
    if constexpr (!std::is_same_v<TypeList, void>) {
        using GateImplementation = typename TypeList::Type;
        testKernelInverses<PrecisionT, ParamT, GateImplementation>(re, num_qubits);
        testKernels<PrecisionT, ParamT, typename TypeList::Next>(re, num_qubits);
    }
}

TEMPLATE_TEST_CASE("Test inverse of gate implementations",
                   "[GateImplementations_Inverse]",
                           float, double) {
    std::mt19937 re(1337);

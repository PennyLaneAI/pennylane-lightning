...
TEST_CASE("IndicesUtil::generateBitPatterns", "[IndicesUtil]") {
    ...
    SECTION("Qubit indices {i}") {
        for (size_t i = 0; i < num_qubits; i++) {
            ...
            auto bit_pattern = IndicesUtil::generateBitPatterns({i}, num_qubits);
    ...
    SECTION("Qubit indices {i,i+1,i+2}") {
        ...
        auto bit_pattern_123 = IndicesUtil::generateBitPatterns({1, 2, 3}, num_qubits);
        auto bit_pattern_012 = IndicesUtil::generateBitPatterns({0, 1, 2}, num_qubits);
    ...
    SECTION("Qubit indices {0,2,3}") {
        ...
        auto bit_pattern = IndicesUtil::generateBitPatterns({0, 2, 3}, num_qubits);

        CHECK(bit_pattern == expected);
    }
    SECTION("Qubit indices {3,1,0}") {
        std::vector<size_t> expected{0, 8, 4, 12, 1, 9, 5, 13};
        auto bit_pattern = IndicesUtil::generateBitPatterns({3, 1, 0}, num_qubits);
        CHECK(bit_pattern == expected);
    }
}
...
TEST_CASE("StateVector::getIndicesAfterExclusion", "[StateVector_Nonparam]") {
    ...
    SECTION("Qubit indices {i}") {
        for (size_t i = 0; i < num_qubits; i++) {
            ...
            auto indices = IndicesUtil::getIndicesAfterExclusion({i}, num_qubits);
            CHECK(indices == expected);
        }
    }
    SECTION("Qubit indices {i,i+1,i+2}") {
        ...
        auto indices_123 = IndicesUtil::getIndicesAfterExclusion({1, 2, 3}, num_qubits);
        auto indices_012 = IndicesUtil::getIndicesAfterExclusion({0, 1, 2}, num_qubits);
    ...
    SECTION("Qubit indices {0,2,3}") {
        ...
        auto indices = IndicesUtil::getIndicesAfterExclusion({0, 2, 3}, num_qubits);

        CHECK(indices == expected);
    }
    SECTION("Qubit indices {3,1,0}") {
        std::vector<size_t> expected{2};
        auto indices = IndicesUtil::getIndicesAfterExclusion({3, 1, 0}, num_qubits);

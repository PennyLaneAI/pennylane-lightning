...
using DeviceView = nb::ndarray<float, nb::shape<-1>, nb::c_contig, nb::device::cuda>;
...
void add_tensors(DeviceTensor& out, const DeviceTensor& a, const DeviceTensor& b) {
    // Basic validation
...
DeviceView device_tensor_gpu_view(nb::object self) {
    // Get a reference to the C++ instance from the Python 'self' object. Needed as a handle/owner
...
NB_MODULE(gpu_binding_example_ext, m) {
    ...
    nb::class_<DeviceTensor>(m, "DeviceTensor")
        ...
        .def("gpu_view", &device_tensor_gpu_view, "Returns a zero-copy view of the tensor on the GPU (DLPack compatible).")
        .def("print_data", &DeviceTensor::print_data, "Prints the data currently stored within the GPU.")

        // Needed for Python to be able to print this object
        .def("__repr__", &DeviceTensor::repr, "Returns a string representation of the DeviceTensor.");

    // Bind the function that operates on our DeviceTensors
    m.def("add_tensors", &add_tensors,
        "Adds two DeviceTensors using a CUDA kernel.",
        nb::arg("out"), nb::arg("a"), nb::arg("b"));
}

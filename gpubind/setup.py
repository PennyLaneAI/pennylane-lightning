...
class CMakeBuild(build_ext):
    def build_extension(self, ext: Extension):
        ...
        cmake_args = [f"-DCMAKE_LIBRARY_OUTPUT_DIRECTORY={extdir}", "-DCMAKE_BUILD_TYPE=Debug"]
        ...
        subprocess.run(
            ["cmake", str(Path(__file__).parent.absolute())] + cmake_args, cwd=build_dir, check=True
        )
...
cmake_extension = Extension("gpu_binding_example.gpu_binding_example_ext", sources=[])

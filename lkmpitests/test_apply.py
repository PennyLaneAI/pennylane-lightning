...
class TestApply:  # pylint: disable=missing-function-docstring,too-many-arguments,too-many-positional-arguments
    ...
    @pytest.mark.skip(reason = "State prep with sub-statevector is not yet supported")
    @pytest.mark.parametrize(
        "par, Wires",
        [
...
class TestExpval:
    ...
    def test_expval_single_wire_no_parameters(self, tol, operation, wires, c_dtype):
        ...
        dev_mpi = qml.device("lightning.kokkos", wires=numQubits, mpi=True, c_dtype=c_dtype)
    ...
    def test_expval_multiple_obs(self, obs, tol, c_dtype):
        ...
        dev_cpu = qml.device("lightning.qubit", wires=num_wires, c_dtype=c_dtype)
        dev_mpi = qml.device("lightning.kokkos", wires=num_wires, mpi=True, c_dtype=c_dtype)
    ...
    def test_expval_non_pauli_word_hamiltionian(self, tol):
        ...
        dev_mpi = qml.device("lightning.kokkos", wires=3, mpi=True)
...
@pytest.mark.skip(reason="Shots not yet supported")
class TestGenerateSample:
    ...
    def test_sample_dimensions(self, c_dtype):
        ...
        dev = qml.device("lightning.kokkos", wires=num_wires, mpi=True, c_dtype=c_dtype)
    ...
    def test_sample_values_qnode(self, tol, c_dtype):
        ...
        dev_mpi = qml.device(
            "lightning.kokkos", wires=num_wires, mpi=True, shots=1000, c_dtype=c_dtype
        )
        dev_mpi._statevector.reset_state()
...
def test_integration(returns):
    ...
    dev = qml.device("lightning.kokkos", wires=num_wires, mpi=True, c_dtype=np.complex128)
    ...
    assert np.allclose(j, j_default, atol=1e-7)
...
def test_integration_custom_wires(returns):
    ...
    dev = qml.device("lightning.kokkos", wires=custom_wires, mpi=True, c_dtype=np.complex128)
    ...
    assert np.allclose(j, j_lightning, atol=1e-7)
